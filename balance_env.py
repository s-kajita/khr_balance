...
from genesis.utils.geom import quat_to_xyz, transform_by_quat, inv_quat, transform_quat_by_quat, quat_to_R, euler_to_quat
import numpy as np
...
class BalEnv:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=True):
        ...
        self.commands_limits: tuple[torch.Tensor, torch.Tensor] = tuple(
            ...
            for values in zip(
                self.command_cfg["footR_z_range"],
                self.command_cfg["footL_z_range"],
        ...
        self.left_foot_link_pos = torch.zeros((self.num_envs, 3), device=gs.device, dtype=gs.tc_float)
        self.right_foot_link_pos = torch.zeros((self.num_envs, 3), device=gs.device, dtype=gs.tc_float)

        # クォータニオン初期化
        self.left_foot_link_quat = torch.zeros_like(self.base_quat)
        self.right_foot_link_quat = torch.zeros_like(self.base_quat)

        # 報酬の最大値
        self.rew_limit = torch.tensor(1.0)
    ...
    def step(self, actions):
        ...
        #左足、右足
        self.left_foot_link_pos[:] = self.robot.get_link('l_foot').get_pos()
        self.right_foot_link_pos[:] = self.robot.get_link('r_foot').get_pos()
        #print(self.r_foot_pos[0])
        
        # クォータニオン取得
        self.left_foot_link_quat[:] = self.robot.get_link("l_foot").get_quat()
        self.right_foot_link_quat[:] = self.robot.get_link("r_foot").get_quat()

        # クォータニオンを回転行列に変換
        self.left_foot_rotm = quat_to_R(self.left_foot_link_quat)
        self.right_foot_rotm = quat_to_R(self.right_foot_link_quat)

        # 回転行列のz軸方向の成分抽出
        self.left_foot_rotm_z_vec = self.left_foot_rotm[:, :, 2]
        self.right_foot_rotm_z_vec = self.right_foot_rotm[:, :, 2]

        # ベクトルの成分抽出
        self.left_foot_up_z = self.left_foot_rotm_z_vec[:, 2]
        self.right_foot_up_z = self.right_foot_rotm_z_vec[:, 2]
    ...
    def _reward_tracking_lin_vel(self):
        # Tracking of linear velocity commands (xy axes)    keep zero
    ...
    def _reward_footL_z(self):   
        reward = torch.exp(-2000 * torch.square(self.left_foot_link_pos[:, 2] - (self.commands[:,0] + self.reward_cfg["foot_link_origin_height"])))
        penalize = torch.square(self.left_foot_link_pos[:, 2] - (self.commands[:,0] + self.reward_cfg["foot_link_origin_height"]))
        return reward #penalize

    def _reward_footR_z(self):   
        reward = torch.exp(-2000 * torch.square(self.right_foot_link_pos[:, 2] - (self.commands[:,1] + self.reward_cfg["foot_link_origin_height"])))
        penalize = torch.square(self.right_foot_link_pos[:, 2] - (self.commands[:,1] + self.reward_cfg["foot_link_origin_height"]))
        return reward #penalize
    
    def _reward_feet_orientation(self):
        situation_lfoot_true_ori = self.left_foot_up_z > 0
        situation_rfoot_true_ori = self.right_foot_up_z > 0
        total_situation = situation_rfoot_true_ori & situation_lfoot_true_ori
        reward = torch.min(
            self.rew_limit,
            torch.exp(5.0 * ((self.left_foot_up_z + self.right_foot_up_z)/2 - 1.0))
            )
        return torch.where(total_situation, reward, 0.0) 
    
    def _reward_body_orientation(self):
        penalize = torch.sum(torch.square(self.projected_gravity[:, :2]), dim=1)
        return penalize
    
    def _reward_ang_vel_xy(self):
        penalize = torch.sum(torch.square(self.base_ang_vel[:, :2]), dim=1)
        return penalize
